...
class LocalHistoryMaster(EvaluatorMasterBase):
    """
    ...
    def __init__(self, t_prof, chief_handle, eval_agent_cls):
        super().__init__(t_prof=t_prof, eval_env_bldr=rl_util.get_env_builder(t_prof=t_prof), chief_handle=chief_handle,
                         eval_type="History_Winnings", log_conf_interval=True)

        self._args = t_prof.module_args["history"]
    ...
    def _load_past_agent_weights(self, iter_nr):
        iter_interval = self._args.iter_interval
        iter_to_load = (iter_nr // iter_interval) * iter_interval
        if iter_to_load == iter_nr:
            iter_to_load -= iter_interval
        if iter_to_load <= 0:
            return False
        file_path = ospj(self._t_prof.path_agent_export_storage,
                         str(self._t_prof.name), str(iter_to_load), 'eval_agent.pkl')
        state = load_pickle(path=file_path)
        for i in range(self._env_bldr.N_SEATS):
            if i != self._REFERENCE_AGENT:
                self._eval_agents[i].load_state_dict(state)
        
        return True

    def evaluate(self, iter_nr):
        """ assumes same action space between all eval agents!! """
        ready = self._load_past_agent_weights(iter_nr)
        if not ready:
            return
