...
class FlatNonHULimitPokerHistoryWrapper(Wrapper):
    """
    ...
    def _reset_state(self, **kwargs):
        self._action_count_this_round = [0] * self.env_bldr.N_SEATS  # one per player
    ...
    def _pushback(self, env_obs=None):
        ...
        # Action structure is [FOLD/CALL/RAISE, bet_size, current_player.seat_id.
        ACTION_TYPE = 0
        BET_SIZE = 1
        SEAT_ID = 2
        _last_a = self.env.last_action[ACTION_TYPE]
        if _last_a is not None:
            _last_actor = self.env.last_action[SEAT_ID]
            ...
            if self.env.current_round != self._game_round_last_tick:
                ...
                self._action_count_this_round = [0] * self.env_bldr.N_SEATS
    ...
    def print_obs(self, wrapped_obs=None):
        ...
        action_sequence = wrapped_obs[self.env_bldr.pub_obs_size - self._action_vector_size:]
